...
class NNSegmentViewZero: UIView {

    /// 必须在设置type之前,type触发布局效果
    var indicatorHeight: CGFloat = 0.5
    
    private var viewBlock: ((NNSegmentViewZero,UISegmentedControl) -> Void)?
    ...
    lazy var segmentCtl: UISegmentedControl = {
        var view = UISegmentedControl.create( .zero, items: ["item0","item1","item2"], selectedIdx: 0, type: 2);
