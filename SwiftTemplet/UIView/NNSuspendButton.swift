...
class NNSuspendButton: UIButton {
    ...
    var superViewRect: CGRect = UIScreen.main.bounds;
...
//        case .began:
        ...
        case .ended:
            ...
            if stopPoint.x - frame.width*0.5 - insets.left <= 0 {
                stopPoint = CGPoint(x: frame.width*0.5 + insets.left,
                                    y: stopPoint.y)
            }
            
            if stopPoint.x + frame.width*0.5 + insets.right >= superViewRect.width {
                stopPoint = CGPoint(x: superViewRect.maxX - frame.width*0.5 - insets.right,
                                    y: stopPoint.y)
            }
            
            if stopPoint.y - frame.height*0.5 - insets.top <= 0 {
                stopPoint = CGPoint(x: stopPoint.x,
                                    y: frame.height*0.5 + insets.top)
            }
            
            if stopPoint.y + frame.height*0.5 + insets.bottom >= superViewRect.height {
                stopPoint = CGPoint(x: stopPoint.x,
                                    y: superViewRect.height - frame.height*0.5 - insets.bottom)
            }
