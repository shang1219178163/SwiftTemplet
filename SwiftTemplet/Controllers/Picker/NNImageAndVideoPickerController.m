//
//  NNImageAndVideoPickerController.m
//  SwiftTemplet
//
...
@interface NNImageAndVideoPickerController ()<TZImagePickerControllerDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate>
...
@implementation NNImageAndVideoPickerController
...
- (instancetype)init{
    ...
    if (self) {
        ...
        _showTakePhoto = true;
        _showTakeVideo = false;
        _sortAscending = false;
        _allowPickingImage = true;
        _allowPickingGif = false;
        _allowPickingOriginalPhoto = true;
        _allowPickingVideo = false;
        _allowPickingMuitlpleVideo = false;
...
- (void)pushTZImagePickerController {
    if (self.maxCount <= 0) {
        [self showAlertTitle:nil message:@"请设置最大选择个数 maxCount" actionTitles:@[@"知道了"] handler: nil];
        return;
    }
...
//     imagePickerVc.oKButtonTitleColorNormal = [UIColor systemBlueColor];
    // imagePickerVc.navigationBar.translucent = NO;
...
//    imagePickerVc.cropRect = CGRectMake(left, top, widthHeight, widthHeight);
    ...
    [imagePickerVc setNavLeftBarButtonSettingBlock:^(UIButton *leftButton){
        ...
        [leftButton setImageEdgeInsets:UIEdgeInsets(0, -10, 0, 20)];
    }];
...
#pragma mark - 到这里为止
    ...
    [imagePickerVc setDidFinishPickingPhotosHandle:^(NSArray<UIImage *> *photos, NSArray *assets, BOOL isSelectOriginalPhoto) {
        if (self.didFinishPickerBlock) {
            self.didFinishPickerBlock(photos, assets, isSelectOriginalPhoto);
        }
    }];
...
- (void)takePhoto {
    ...
    if (authStatus == AVAuthorizationStatusRestricted || authStatus == AVAuthorizationStatusDenied) {
        ...
        UIAlertController *alertVC = [UIAlertController alertControllerWithTitle:@"无法访问相册"
...
//    }];
    ...
    if ([UIImagePickerController isSourceTypeAvailable: UIImagePickerControllerSourceTypeCamera]) {
        ...
        UIWindow *keyWindow = UIApplication.sharedApplication.delegate.window;
        [keyWindow.rootViewController presentViewController:_imagePickerVc animated:YES completion:nil];
    } else {
