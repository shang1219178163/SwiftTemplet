...
class BNPhotosView: UIView, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    var viewBlock: ((BNPhotosView, [UIImage]) -> Void)?
...
//                    DDLog(sender.tag)
    ...
    func block(_ action: @escaping ((BNPhotosView, [UIImage]) -> Void)) -> Void {
        viewBlock = action;
    }
    ...
    func showAlertController() -> Void {
        let items = ["从相册选择", "拍照"]
        UIAlertController.showSheet(nil, items: items) { (action) in
