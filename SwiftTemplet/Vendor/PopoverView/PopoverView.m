...
- (void)layoutSubviews{
    ...
    _tableView.frame = CGRectMake(0, _isUpward ? self.arrowSize.height : 0,
                                  CGRectGetWidth(self.bounds), CGRectGetHeight(self.bounds) - self.arrowSize.height);
}
...
- (void)showToPoint:(CGPoint)toPoint{
    ...
    CGFloat currentW = _contentWidth > 0 ? _contentWidth : [self calculateMaxWidth]; // 宽度通过计算获取最大值
    ...
    // 箭头高度
    currentH += self.arrowSize.height;
    ...
    if (!_isUpward) {
        ...
        if (_lockArrowUp) {
            currentY += CGRectGetHeight(_pointView.bounds) + 5;
        }
    }
    if (_arrowStyle == PopoverViewArrowStyleNone) {
        currentY -= self.arrowSize.height;
    }
    ...
    CGFloat maskTop = _isUpward ? self.arrowSize.height : 0; // 顶部Y值
    CGFloat maskBottom = _isUpward ? currentH : currentH - self.arrowSize.height; // 底部Y值
    ...
    // 箭头向上时的箭头位置
    if (_isUpward) {
        [maskPath addLineToPoint:CGPointMake(arrowPoint.x - arrowWidth/2, self.arrowSize.height)];
        // 菱角箭头
        if (_arrowStyle == PopoverViewArrowStyleTriangle) {
            ...
            if (_arrowStyle != PopoverViewArrowStyleNone) {
                [maskPath addQuadCurveToPoint:CGPointMake(arrowPoint.x - arrowCornerRadius, arrowCornerRadius)
                                 controlPoint:CGPointMake(arrowPoint.x - arrowWidth/2 + arrowBottomCornerRadius, self.arrowSize.height)];
                [maskPath addQuadCurveToPoint:CGPointMake(arrowPoint.x + arrowCornerRadius, arrowCornerRadius)
                                 controlPoint:arrowPoint];
                [maskPath addQuadCurveToPoint:CGPointMake(arrowPoint.x + arrowWidth/2, self.arrowSize.height)
                                 controlPoint:CGPointMake(arrowPoint.x + arrowWidth/2 - arrowBottomCornerRadius, self.arrowSize.height)];
            }
    ...
    // 箭头向下时的箭头位置
    if (!_isUpward) {
        [maskPath addLineToPoint:CGPointMake(arrowPoint.x + arrowWidth/2, currentH - self.arrowSize.height)];
        // 菱角箭头
        if (_arrowStyle == PopoverViewArrowStyleTriangle) {
            [maskPath addLineToPoint:arrowPoint];
            [maskPath addLineToPoint:CGPointMake(arrowPoint.x - arrowWidth/2, currentH - self.arrowSize.height)];
            ...
            if (_arrowStyle != PopoverViewArrowStyleNone) {
                [maskPath addQuadCurveToPoint:CGPointMake(arrowPoint.x + arrowCornerRadius, currentH - arrowCornerRadius)
                                 controlPoint:CGPointMake(arrowPoint.x + arrowWidth/2 - arrowBottomCornerRadius, currentH - self.arrowSize.height)];
                [maskPath addQuadCurveToPoint:CGPointMake(arrowPoint.x - arrowCornerRadius, currentH - arrowCornerRadius)
                                 controlPoint:arrowPoint];
                [maskPath addQuadCurveToPoint:CGPointMake(arrowPoint.x - arrowWidth/2, currentH - self.arrowSize.height)
                                 controlPoint:CGPointMake(arrowPoint.x - arrowWidth/2 + arrowBottomCornerRadius, currentH - self.arrowSize.height)];
            }
        }
    }
...
- (void)showToView:(UIView *)pointView withActions:(NSArray<PopoverAction *> *)actions{
    ...
    // 弹窗在 pointView 底部
        ...
        if (_lockArrowUp) {
            toPoint.y += CGRectGetHeight(pointView.bounds);
        }
    }
